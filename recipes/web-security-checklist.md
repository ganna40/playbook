...
### AI가 바로 할 수 있는 일
- 코드에서 비밀값, 하드코딩 값, 관리자 우회 경로를 검색한다
- `/admin`, `/internal`, 관리자 API 접근 제어를 점검하고 수정한다
- `HttpOnly`, `Secure`, `SameSite` 쿠키 기반 인증 구조를 구현한다
- 상세 에러 숨김, 감사 로그, 로그인 rate limit, 보안 헤더를 추가한다
- CORS, source map, 업로드 제한, 권한 검사 로직을 수정한다
- PostgreSQL RLS 정책 초안 또는 MySQL/MariaDB owner 필터 코드를 만든다
- Nginx, Apache, FastAPI, Express 같은 서버 설정 파일 수정안을 만든다
- 빌드, 테스트, 배포 스크립트, 점검 체크리스트, 운영 문서를 작성한다

### 사람이 직접 해야 하는 일
- Cloudflare 대시보드에서 프록시, SSL, HTTPS 옵션을 실제로 켠다
- 운영 `.env`, GitHub Secrets, AWS Secrets Manager에 실제 비밀값을 넣는다
- 관리자 허용 IP, VPN, SSO, MFA 정책을 결정하고 적용한다
- DNS, 도메인, 인증서 소유권과 계정 접근 권한을 관리한다
- 백업 보관 주기, 복구 승인, 운영 차단 정책을 최종 결정한다
- 서버 접근 권한, IAM, 계정 소유권 같은 조직 권한을 승인한다

### 같이 해야 하는 일
- AI가 설정 파일과 코드 변경을 준비하고, 사람은 실제 운영값과 접근 권한을 입력한다
- AI가 서버 설정을 점검하고 수정안을 적용하며, 사람은 배포 승인과 영향 판단을 한다
- AI가 복구 절차를 문서화하고 테스트 스크립트를 만들며, 사람은 실제 복구 기준을 승인한다

## 실행 순서

### 1. 사람이 먼저 준비할 것
- [ ] 운영 저장소 또는 서버 접근 권한
- [ ] Cloudflare, AWS, GitHub 등 필요한 계정 접근 권한
- [ ] 실제 도메인과 배포 대상 서버 정보
- [ ] 운영 비밀값을 넣을 위치
...
- [ ] 수정 코드 작성
- [ ] 빌드와 테스트
- [ ] 배포 가능한 설정 파일과 점검 보고서 작성

### 3. 사람이 마지막에 할 것
- [ ] 실제 비밀값 입력
- [ ] Cloudflare / SSL / DNS 옵션 반영
- [ ] MFA, 허용 IP, 계정 권한 설정
- [ ] 최종 운영 배포 승인
- [ ] 배포 후 브라우저와 서버 화면에서 체크리스트 확인
...
## AI에게 바로 맡기는 프롬프트 - 역할 분리형

```text
이 프로젝트에서 웹서비스 보안 점검과 수정 작업을 해줘.

작업 방식:
1. 먼저 "AI가 바로 처리할 수 있는 항목"과 "사람이 직접 설정해야 하는 항목"을 구분해서 보여줘.
2. 그 다음 AI가 처리할 수 있는 항목은 바로 코드/설정/문서 수정까지 진행해줘.
3. 사람이 직접 해야 하는 항목은 실제 설정 위치, 설정 방법, 확인 방법까지 체크리스트로 정리해줘.
4. 마지막에는 "지금 바로 내가 해야 하는 것"만 따로 짧게 정리해줘.
