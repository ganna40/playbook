...
| **Lucide-React** | React 아이콘 팩 | vibejob, infra-quote |
...
| **lottie-web** | Lottie JSON 벡터 애니메이션 재생 | quit-calculator |
| **View Transitions API** | 네이티브 앱 같은 화면 전환 | quit-calculator |
...
| **Web Share API** | 모바일 네이티브 공유 | salary, pong, mz, amlife, tok-iq | |
| **Clipboard API** | URL/텍스트 복사 | salary, pong, mz, amlife, hexaconsulting, tok-wrapped, tok-iq | |
...
| **httpx** | Python 비동기 HTTP 클라이언트 | wp-mcp, saju |
| **lunar_python** | 한중 음양력 변환 + 만세력 (팔자/대운/세운 계산) | saju |
| **Web Audio API** | OscillatorNode 효과음 (외부 파일 불필요) | love, tok-iq | |
...
| **excelize** | Go XLSX 파서 (xuri/excelize v2) | amazon-report-hub | |
...
| 기술 | salary | pong | mz | amlife | food | tarot | psycho-bot | telbot | collab-tool | product-j | error-auto | human2 | dictionary | rackops | hexalounge | hexaconsulting | tok-wrapped | tok-iq | vibejob | poli | whisper | wp-mcp | love | ideal | quit-calc | infra-quote | saju | n-mon | a-hub |
|------|:------:|:----:|:--:|:------:|:----:|:-----:|:----------:|:------:|:-----------:|:---------:|:----------:|:------:|:----------:|:-------:|:----------:|:-------------:|:-----------:|:------:|:-------:|:----:|:-------:|:-------:|:----:|:-----:|:---------:|:-----------:|:----:|:-----:|:-----:|
| **프론트엔드** | | | | | | | | | | | | | | | | | | | | | | | | | | | | |  |
| Tailwind CDN | ✅ | | | | | | | | | | | | | | ✅ | ✅ | | | |  | | | | | ✅ | | | |  |
| Tailwind v3 빌드 | | | | | ✅ | | | | | | | | | ✅ | | | | | |  | | | | | | | | |  |
...
| 구문 하이라이팅 | | | | | | | | | | | | | ✅ | | | | | | |  | | | | | | | | |  |
| go:embed 단일 바이너리 | | | | | | | | | | | | | ✅ | | | | | | |  | | | | | | | | | ✅ |
| Embedded PostgreSQL (자동) | | | | | | | | | | | | | | | | | | | | | | | | | | | | | ✅ |
| 듀얼 모드 (HTTP/IPC) | | | | | | | | | | | | | | | | | | | | | | | | | | | | | ✅ |
| JSONB 스키마리스 저장 | | | | | | | | | | | | | | | | | | | | | | | | | | | | | ✅ |
| CSV/TSV/XLSX 파싱 (excelize) | | | | | | | | | | | | | | | | | | | | | | | | | | | | | ✅ |
| 채널 자동 감지 (US/CA FBA) | | | | | | | | | | | | | | | | | | | | | | | | | | | | | ✅ |
| pgx.Batch 벌크 삽입 (1000건) | | | | | | | | | | | | | | | | | | | | | | | | | | | | | ✅ |
| 비주얼 랙 다이어그램 | | | | | | | | | | | | | | ✅ | | | | | |  | | | | | | | | |  |
| 네트워크 토폴로지 (D3.js) | | | | | | | | | | | | | | ✅ | | | | | |  | | | | | | | | |  |
| SSH/SNMP 자동 감지 | | | | | | | | | | | | | | ✅ | | | | | |  | | | | | | | | |  |
| SSH Console (웹 터미널) | | | | | | | | | | | | | | ✅ | | | | | |  | | | | | | | | |  |
...
| 혼합 질문 타입 (4종) | | | | | | | | | | | | | | | | | | | | | | | ✅ | | | | | |  |
| ?art= JWT 카페 접근 | | | | | | | | | | | | | | | | | | | | | | | | | | | | ✅ |  |
| iframe Vue SPA 댓글 크롤링 | | | | | | | | | | | | | | | | | | | | | | | | | | | | ✅ |  |
| subprocess Playwright 분리 | | | | | | | | | | | | | | | | | | | | | | | | | | | | ✅ |  |
| 24H 자동 재검증 | | | | | | | | | | | | | | | | | | | | | | | | | | | | ✅ |  |
| 수동 노출확인 + 정산 이관 | | | | | | | | | | | | | | | | | | | | | | | | | | | | ✅ |  |
| **테마** | | | | | | | | | | | | | | | | | | | | | | | | | | | | |  |
| 라이트 | ✅ | | ✅ | ✅ | ✅ | | | | ✅ | | | | ✅ | | ✅ | ✅ | | | ✅ | | ✅ | | | | ✅ | | | |  |
...
조합: BASE + SCREEN + GRADE + REVEAL + SHARE + CAPTURE + SOUND + HAPTIC + STYLE-DARK + WebWorker + 웩슬러5지표엔진

카카오톡 .txt 내보내기 파일 업로드
  → Web Worker로 메인 스레드 보호하며 파싱
  → 모바일+PC 포맷 정규식 파싱 (tok-wrapped 공유)
  → 웩슬러(WAIS-V) 5대 인지지표 측정:
    VCI(언어이해), FRI(유동추론), WMI(작업기억), PSI(처리속도), VSI(시공간)
  → 인지 사전 6종: 격식체, 고급어휘, 조건추론, 문제해결, 맥락참조, 언어깊이
  → 그룹 적응형 스코어링 S()/R() + z-score+tanh
  → raw 0~100 → 웩슬러 Index 55~145 → FSIQ
  → 10장 카드 시퀀스 (요약+5지표+펜타곤+IQ공개+랭킹+공유)
  → SVG 5축 펜타곤 차트
  → html2canvas 이미지 저장 + 클립보드 복사

프론트: 순수 CSS (다크 뉴럴 테마)
폰트:   Pretendard Variable (CDN)
특수:   웩슬러 5지표 스코어링, 인지 사전 6종, 그룹 적응형 점수, 펜타곤 차트
서버:   없음 (100% 정적, 대화 데이터 브라우저 밖으로 안 나감)
```
...
### quit-calculator - 퇴사 타이밍 계산기
```
유형: 계산기
GitHub: github.com/ganna40/-severance_pay
조합: BASE + CALC + GRADE + SHARE + CAPTURE + JSPDF + PWA + VIEW-TRANSITIONS + CONFETTI + LOTTIE + STYLE-LIGHT

입사일/월급/연차/상여금 입력
  → 퇴직금 계산 (1일 평균임금 × 30 × 재직일수/365)
  → 연차수당 계산 (미사용 연차 × 1일 통상임금)
  → 최적 퇴사일 추천 (30일 범위 탐색)
  → S~F 5등급 판정 + 컨페티 + Lottie 축하
  → PDF 리포트 (html2canvas→jsPDF 멀티 페이지)
  → 이미지 캡처 (html2canvas PNG)
  → View Transitions 화면 전환
  → PWA (오프라인 지원, 홈 화면 추가)

데이터: 인라인 (JS 내 직접)
폰트:   Pretendard (CDN)
CSS:    Tailwind CDN 3.4.1
특수:   근로기준법 기준 계산, 평균임금 실제 역일수, 통상임금 ÷209×8
신규기술: jsPDF, PWA, View Transitions API, canvas-confetti, lottie-web
```
...
유형: 업무 도구 (인프라 견적서 빌더)
URL:  (로컬 전용)
스택: React 19 + TypeScript + Vite 7 + Tailwind CSS v4 + @xyflow/react + @dnd-kit/core

카탈로그 CRUD (그룹/아이템/스펙필드 직접 생성)
  → 드래그 앤 드롭으로 캔버스에 아이템 배치
  → React Flow 노드 연결 → 아키텍처 다이어그램
  → 스펙별 비용 자동 계산 (basePrice × multiplier + unit × price)
  → 인쇄용 견적서 (그룹별 소계 + 총합)

프론트: React 19, Tailwind CSS v4 (@theme), @xyflow/react, @dnd-kit/core, Lucide-React
백엔드: Express + tsx (JSON 파일 CRUD 서버, port 8000)
데이터: data/catalog.json + data/quotations/*.json
폰트:   시스템 폰트 (에어갭 환경)
특수:   커스텀 Flow 노드 2종 (InfraNode/ServiceBadgeNode), 3컬럼 카탈로그 매니저
```
